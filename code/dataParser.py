...
class Parser:
    def __init__(self, outputFile) -> None:
        ...
        # MultiIndex
        self.HEADER_FIRST_LEVEL = [
            "", "", *list(chain(*[[year for _ in range(len(constants.SEMESTERS))] for year in constants.YEARS]))]
        self.HEADER_SECOND_LEVEL = [
            "", "HF/NF", *list(chain(*[["Fachsemester" for _ in range(len(constants.SEMESTERS))] for _ in range(len(constants.YEARS))]))]
    ...
    def __createPandasStructureSingleFaculty(self, subjectData: List[str]) -> Tuple[List[str], List[str]]:
        ...
        totalMainSubject = list(chain.from_iterable([data.split(" ")[1:]
                                for data in subjectData if re.match(constants.HF_PATTERN, data)]))[0:-1]
        totalMinorSubject = list(chain.from_iterable([data.split(" ")[1:]
                                 for data in subjectData if re.match(constants.NF_PATTERN, data)]))[0:-1]
        ...
        if totalMainSubject is None or len(totalMainSubject) == 0:
            totalMainSubject = ["0" for _ in range(len(constants.SEMESTERS))]
        if totalMinorSubject is None or len(totalMinorSubject) == 0:
