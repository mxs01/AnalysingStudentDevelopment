...
Our central contribution is a Vector autoregressive model (VAR) discussed in \Cref{sec:methods} that is built using just mentioned data. Combined with a
Partial autocorrelation function (PACF) the optimal value for lag
is determined, see \Cref{sec:methods} \textcolor{red}{subsection}, to ensure that relevant structure in the data is captured by the model.
After verifying that the VAR model functions as expected and testing the model on a test set, \Cref{sec:methods} \textcolor{red}{subsection}, we
...
% Mabe focus here on what we did with the data
% I.e. inflation adjustment
% flattening G8 G9 conflict
