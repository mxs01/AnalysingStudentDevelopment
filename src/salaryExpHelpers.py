"""Helper functions for salary augmentation experiments"""
...
def plot(data: np.ndarray, forecast: np.ndarray, ogForecast: np.ndarray, years: List[str], name: str, lags: int) -> plt.Figure:
    ...
    plt.title('Enrolled STEM students with altered salary')
    ...
    ax2.set_ylabel('Gross salary in EUR', color=COL_SALARY)
#     ax2.set_ylabel(np.arange(3800, 4200, 100))
